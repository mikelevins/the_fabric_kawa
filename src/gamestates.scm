...
(defclass PlayGameState (FabricGameState)
  (slots:
   (camera-node init-form: #!null getter: getCameraNode setter: setCameraNode)
   ...
   ((prepareToAttach mgr::AppStateManager client::FabricClient)
    (unless (*:getInitialized (this))
      (let* ((client::FabricClient (*:getApp (this)))
             ...
             (body-texture (*:loadTexture asset-manager "Textures/Venus.jpg"))
             (rotation (Quaternion))
             (TextureMode Sphere:TextureMode)
             (Projected TextureMode:Projected)
             (pitch-axis (Vector3f 1 0 0))
             (camera (*:getCamera client)))
        (*:setFrustumFar camera 20000)
        (set! camera-node (CameraNode "Camera" camera))
        (set! celestial-body (Sphere 128 128 2048.0))
        (set! body-rotator (RotateControl 0.0 0.0 0.0))
        (set! body-pivot (com.jme3.scene.Geometry "Center" celestial-body))
        ;;(*:fromAngleAxis rotation (* 0 (/ PI 2)) pitch-axis)
        (*:setLocalRotation body-pivot rotation)
        (*:setLocalTranslation body-pivot 0 0 -6000)
        (*:setLocalTranslation camera-node (Vector3f 0 0 0))
        (*:lookAt camera-node (*:getLocalTranslation body-pivot) Vector3f:UNIT_Y)
        (set! sky
