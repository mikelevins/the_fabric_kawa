...
(define (init-hud app ::SimpleApplication name-string)
  ...
  (let* ((gui-node (client-gui-node app))
         ...
         (name-panel (Container "glass"))
         (asset-manager (get-asset-manager))
         (name-font (@ 'loadFont asset-manager "Interface/Fonts/Orbitron36.fnt"))
         (nameplate (Label name-string "glass")))
    (@ 'set (@ 'getSelector styles Panel:ELEMENT_ID "glass")
            "background" background)
    (@ 'setLocalTranslation name-panel 10 50 0)
    (@ 'attachChild gui-node name-panel)
    (@ 'setFont nameplate name-font)
    (@ 'setFontSize nameplate 36)
    (@ 'setColor nameplate ColorRGBA:Green)
    (@ 'addChild name-panel nameplate)))
