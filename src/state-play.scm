...
(define (%play-state-handle-analog-event state::PlayState name value tpf)
  (let* ((client::FabricClient (the-client))
    ...
    (cond
     ...
     ((*:equals name "MouseDragRight")(when right-button-down? (rotate-node-right! node (* 4.0 tpf))))
     ((*:equals name "MouseDragLeft")(when right-button-down? (rotate-node-left! node (* 4.0 tpf))))
     ((*:equals name "MouseDragUp")(when right-button-down? (rotate-node-down! node (* 2.0 tpf))))
     ((*:equals name "MouseDragDown")(when right-button-down? (rotate-node-up! node (* 2.0 tpf))))
     (else #f))))
