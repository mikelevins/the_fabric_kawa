...
(define (%play-state-cleanup state::PlayState)
  (let* ((client::FabricClient (the-client))
    ...
    (*:detachChild root state:character-nameplate)
...
(define (%play-state-initialize state::PlayState)
  (let* ((client::FabricClient (the-client))
    ...
    (*:setLocalTranslation model (Vector3f 0.0 -4000.0 35000.0))
...
(define (%play-state-handle-analog-event state::PlayState name value tpf)
  (let* ((client::FabricClient (the-client))
    ...
    (cond
     ...
     ((*:equals name "KeyS")(move-node-backward! client node (* 0.5 speed tpf)))
     ((*:equals name "KeyD")(move-node-right! client node (* 0.5 speed tpf)))
     ((*:equals name "KeyA")(move-node-left! client node (* 0.5 speed tpf)))
     ((*:equals name "KeyQ")
      (roll-node-left! node (* state:roll-speed tpf)))
     ((*:equals name "KeyE")
      (roll-node-right! node (* state:roll-speed tpf)))
     ((*:equals name "KeySPACE")(move-node-up! client node (* 0.5 speed tpf)))
     ((*:equals name "KeyX")(move-node-down! client node (* 0.5 speed tpf)))
     ((*:equals name "MouseDragRight")
      (when right-button-down? (rotate-node-right! node (* state:yaw-speed tpf))))
     ((*:equals name "MouseDragLeft")
      (when right-button-down? (rotate-node-left! node (* state:yaw-speed tpf))))
     ((*:equals name "MouseDragUp")
      (when right-button-down? (rotate-node-down! node (* state:pitch-speed tpf))))
     ((*:equals name "MouseDragDown")
      (when right-button-down? (rotate-node-up! node (* state:pitch-speed tpf))))
     (else #f))))
...
(define-simple-class PlayState (FabricClientState)
  ...
  (pitch-speed type: float init-form: 3.0)
  (yaw-speed type: float init-form: 6.0)
  (roll-speed type: float init-form: 0.5)
  ;; methods
