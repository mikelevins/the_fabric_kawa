...
(define (make-portals-augment)
  (let* ((asset-manager (get-asset-manager))
         (portals-mat::Material (Material asset-manager "Common/MatDefs/Misc/Unshaded.j3md"))
         (color (ColorRGBA 0.8 0.5 1 0.75))
         (glow-color (ColorRGBA 0.8 0.7 1 0.8))
         (pivot (Node "CharacterAugment"))
         (pivot0 (Node "ModulePivot0"))
         (pivot1 (Node "ModulePivot1")))
    (*:setColor portals-mat "Color" color)
    (*:setColor portals-mat "GlowColor" glow-color)
    (let* ((blendMode RenderState:BlendMode))
      (*:setBlendMode (*:getAdditionalRenderState portals-mat) blendMode:Alpha))
    (let* ((portals-module0 (PQTorus 2 -3 0.3 0.125 64 64))
           (portals-module1 (PQTorus 2 -3 0.3 0.125 64 64))
           (geom0::Geometry (Geometry "Module0" portals-module0))
           (geom1::Geometry (Geometry "Module1" portals-module1))
           (bucket RenderQueue:Bucket)
           (rotation::Quaternion (Quaternion))
           (pitch-axis (Vector3f 1 0 0)))
      (*:fromAngleAxis rotation (/ PI 2) pitch-axis)
      (*:setLocalRotation geom0 rotation)
      (*:setLocalRotation geom1 rotation)
      (*:setMaterial geom0 portals-mat)
      (*:setMaterial geom1 portals-mat)
      (*:setQueueBucket geom0 bucket:Transparent)
      (*:setQueueBucket geom1 bucket:Transparent)
      (*:setLocalTranslation geom0 0.0 0.0 0.0)
      (*:setLocalTranslation geom1 0.0 0.0 0.0)
...
(define (make-turrets-augment)
  (let* ((asset-manager (get-asset-manager))
    ...
    (let* ((cannon-module0 (WireBox 0.25 0.25 0.25))
           (cannon-module1 (WireBox 0.25 0.25 0.25))
           (cannon-module2 (WireBox 0.25 0.25 0.25))
           (cannon-module3 (WireBox 0.25 0.25 0.25))
