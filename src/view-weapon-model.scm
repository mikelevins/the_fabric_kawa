...
(define (make-impulse-weapon)
  (let* ((asset-manager (get-asset-manager))
         ...
         (color (ColorRGBA 0.6 0.6 0.7 0.8))
         (bright-color (ColorRGBA 1 1 1 0.8))
         (dim-color (ColorRGBA 0.6 0.6 0.7 0.8))
         (glow-color (ColorRGBA 1 1 1 0.9))
         (pivot (Node "CharacterWeapon"))
         (pivot0 (Node "ModulePivot0"))
         (pivot1 (Node "ModulePivot1"))
         (flicker0::FlickerControl (FlickerControl dim-color bright-color 40))
         (flicker1::FlickerControl (FlickerControl dim-color bright-color 40)))
    ...
    (let* ((impulse-module0 (Sphere 32 32 0.2))
      ...
      (*:addControl geom0 flicker0)
      (*:addControl geom1 flicker1)
      (*:attachChild pivot pivot0)
      (*:attachChild pivot pivot1)
      pivot)))
