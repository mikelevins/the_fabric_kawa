...
(define (make-jupiter)
  (let* ((jupiter (make-celestial-body "Jupiter" 8192 0.0075))
         ...
         ;; top cluster of characters
         (npc1::FabricCharacter (make-random-character))
         (npc1-model (make-character-model npc1))
         (npc2::FabricCharacter (make-random-character))
         (npc2-model (make-character-model npc2))
         (npc3::FabricCharacter (make-random-character))
         (npc3-model (make-character-model npc3))
         ;; right cluster
         (npc4::FabricCharacter (make-random-character))
         (npc4-model (make-character-model npc4))
         (npc5::FabricCharacter (make-random-character))
         (npc5-model (make-character-model npc5))
         )
    ...
    ;; top cluster of characters
    (*:setLocalTranslation npc1-model 0.0 800.0 0.0)
    (*:setLocalTranslation npc2-model 10.0 820.0 0.0)
    (*:setLocalTranslation npc3-model 25.0 810.0 0.0)
    (*:attachChild volvox npc1-model)
    (*:attachChild volvox npc2-model)
    (*:attachChild volvox npc3-model)
    ;; right cluster
    (*:setLocalTranslation npc4-model 800.0 0.0 0.0)
    (*:setLocalTranslation npc5-model 815.0 0.0 0.0)
    (*:attachChild volvox npc4-model)
    (*:attachChild volvox npc5-model)
    ;; return the assembly
    jupiter-pivot))
